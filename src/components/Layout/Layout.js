...
function Layout(props) {
  ...
  const pictures = (
    <div className="imgcontainer flexc">
      <img className="logo" src={Image} alt="logo" />
    </div>
  );
  const linkovi = ["Početna", "Usluge", pictures, "O nama", "Kontakt"].map(
    (item, i) =>
      i !== 4 ? (
        <NavLink
          onClick={scrollontop}
  ...
  return (
    <div
      className="flexs layout"
      style={{
        opacity: scroll ? 1 : 0.9,
        height: props.openmenu ? "150px" : "",
        transform: scroll && windowwidth > 600 ? "translateY(-100px)" : "",
