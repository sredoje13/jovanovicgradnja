...
function Servesutil(props) {
  ...
  const allservescolumn =props.allserves.map((item,i) => (
    ...
    className={classes.list}
     onClick={()=>{dispatch(actions.choseserverindex(i))
     windowwidth<960&&props.scroll()
     
     }}
    sx={{background:'linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.2))',
   }}
    >
      <ListItemButton>
        {serverindex===i&&<ListItemIcon sx={{minWidth:windowwidth>500?'16px':'5px'}}>
          <ArrowForwardIosIcon  sx={{width:windowwidth>350?'25px':'15px'}}/>
        </ListItemIcon>}
        <ListItemText
            primaryTypographyProps={{fontSize: windowwidth>500?'16px':'10px'}} 
