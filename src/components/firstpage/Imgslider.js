  ...
  return (
    <div className="flexcolumn slider2">
      <div className="slider2container flexc">
        <IconButton onClick={previusphoto}>
          <ChevronLeftIcon fontSize={fontzize} sx={{ color: "white" }} />
        </IconButton>
        <img onClick={changephoto} src={props.images[index]} alt="slikaa" />
        <IconButton onClick={changephoto}>
          {" "}
          <ChevronRightIcon fontSize={fontzize} sx={{ color: "white" }} />
        </IconButton>
      </div>
      <div className="flexc">{dots}</div>
    </div>
  );
}
